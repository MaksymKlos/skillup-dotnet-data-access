...
/// <summary>
/// Order aggregate root. Owns its lines and guards all invariants: lines can only
/// change while the order is a draft, an order cannot be placed without lines, and
...
public sealed class Order : AggregateRoot<OrderId>
{
    ...
    {
        return new Order(OrderId.New(), customerId, createdAt);
    }
    ...
    public void AddLine(ProductId productId, int quantity, Money unitPrice)
    {
        ...
        var existingOrderLine = _lines.FirstOrDefault(line => line.ProductId == productId);
        if (existingOrderLine is not null)
        {
            existingOrderLine.ChangeQuantity(existingOrderLine.Quantity + quantity);
        }
        else
        {
